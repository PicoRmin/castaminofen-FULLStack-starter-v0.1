...
import type { SchedulerContext, SchedulerPolicyEvaluationResult, SchedulerTimeProvider } from '../types';
...
abstract class BaseSchedulingPolicy implements SchedulingPolicyContract {
  public constructor(public readonly id: string, public readonly kind: 'fixed-interval' | 'cron-expression' | 'custom', public readonly description: string) {}

  public validate(): void {
    if (!this.id.trim()) {
      throw new InvalidSchedulingPolicy('Policy id is required.', { policyId: this.id, executionStage: 'validation' });
    }
  }

  public abstract evaluate(context: SchedulerContext, trigger: SchedulerTriggerContract, timeProvider: SchedulerTimeProvider): SchedulerPolicyEvaluationResult;
}
...
export class FixedIntervalPolicy extends BaseSchedulingPolicy {
  ...
  public override validate(): void {
    ...
    if (!Number.isFinite(this.intervalMs) || this.intervalMs <= 0) {
      throw new InvalidSchedulingPolicy('Interval must be a positive number.', { policyId: this.id, executionStage: 'validation' });
    }
  }

  public override evaluate(context: SchedulerContext, _trigger: SchedulerTriggerContract, timeProvider: SchedulerTimeProvider): SchedulerPolicyEvaluationResult {
...
export class CronExpressionPolicy extends BaseSchedulingPolicy {
  ...
  public override validate(): void {
    ...
    if (!this.expression.trim()) {
      throw new InvalidSchedulingPolicy('Cron expression is required.', { policyId: this.id, executionStage: 'validation' });
    }
  }
...
export class ExponentialBackoffPolicy extends BaseSchedulingPolicy {
  ...
  public override validate(): void {
    ...
    if (!Number.isFinite(this.baseDelayMs) || this.baseDelayMs <= 0) {
      throw new InvalidSchedulingPolicy('Base delay must be positive.', { policyId: this.id, executionStage: 'validation' });
    }
  }
